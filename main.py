...
from ds_canvas import DSCanvas
...
class MainWindow:
    ...
    def _create_widgets(self):
        ...
        self._ds_canvas = DSCanvas(self._content)
    ...
    def _map_data_structures_to_canvas(self, *args):
        self._ds_canvas.canvas.delete(Tag.ALL_CANVAS_ITEMS)
        parser = EntryParser(entry=self._example_value.get())
        parser.parse()
        factory = DataStructureFactory(
            canvas_width=self._ds_canvas.canvas.winfo_width(),
            canvas_height=self._ds_canvas.canvas.winfo_height()
        )
        data_structures = factory.create(entries=parser.result, ds_type=self._ds_type_value.get())
        for ds in data_structures:
            ds.map_to_canvas(self._ds_canvas.canvas)
